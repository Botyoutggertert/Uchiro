...
type AuthTab = 'signin' | 'signup' | 'forgot' | 'telegram';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const [telegramStep, setTelegramStep] = useState<'enter-username' | 'needs-link' | 'enter-code'>('enter-username');
  const [telegramCooldown, setTelegramCooldown] = useState(0);
  const [customerBotUsername, setCustomerBotUsername] = useState('uchirostore_bot');
  ...
  useEffect(() => {
    ...
    fetch('/api/state')
      .then((res) => res.json())
      .then((data) => {
        const uname = data?.data?.settings?.verificationBotUsername;
        if (uname) setCustomerBotUsername(String(uname).replace(/^@/, ''));
      })
      .catch(() => {});
  }, [isOpen]);
  ...
  const resetForm = () => {
    ...
    setTelegramUsername('');
    setTelegramCode('');
    setTelegramStep('enter-username');
    setTelegramCooldown(0);
  };
  ...
  // Google Sign-In: shares the same profile-sync/admin-check flow as email sign-in.
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      const profile = await syncOrInitFirestoreProfile(fbUser);
      const effectiveUsername = profile.username || fbUser.displayName || fbUser.email?.split('@')[0] || 'Player';
      const effectiveRefCode = ensureRandomReferralCode(profile.referralCode);
  ...
  // Telegram Login: request a 6-digit code sent via the store bot
  const handleTelegramRequestCode = async () => {
    setErrorMessage(null);
    const clean = telegramUsername.trim().replace(/^@/, '');
    if (!clean) {
      setErrorMessage(lang === 'KM' ? 'សូមបញ្ចូល Telegram username' : 'Enter your Telegram username');
      return;
    }
    setIsLoading(true);
    try {
      const result = await api.requestTelegramLoginCode(clean);
      if (result.needsLink) {
        setTelegramStep('needs-link');
      } else if (result.success) {
        setTelegramStep('enter-code');
        setTelegramCooldown(30);
      } else {
  ...
  // Telegram Login: verify the 6-digit code and complete sign-in
  const handleTelegramVerifyCode = async () => {
    setErrorMessage(null);
    const clean = telegramUsername.trim().replace(/^@/, '');
    if (!telegramCode.trim() || telegramCode.trim().length !== 6) {
      setErrorMessage(lang === 'KM' ? 'សូមបញ្ចូលលេខកូដ ៦ខ្ទង់' : 'Enter the 6-digit code');
      return;
    }
    setIsLoading(true);
    try {
      const result = await api.verifyTelegramLoginCode(clean, telegramCode.trim());
      if (!result.success || !result.token) {
        setErrorMessage(result.error || 'Invalid code');
        return;
      }

      const fbUser = await loginWithTelegramToken(result.token);
      const profile = await syncOrInitFirestoreProfile(fbUser);
      const effectiveUsername = profile.username || clean;
      const effectiveRefCode = ensureRandomReferralCode(profile.referralCode);

      const adminCheck = await api.verifyAdminUser(profile.email || '', fbUser.uid);
      const userRole: 'customer' | 'admin' = adminCheck.isAdmin || profile.role === 'admin' ? 'admin' : 'customer';
      ...
      setErrorMessage(err?.message || 'Failed to complete Telegram login');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="overflow-y-auto pr-0.5 max-h-[calc(92dvh-1.5rem)]">
            ...
            <>
              ...
              {activeTab !== 'forgot' && (
                <div className="flex rounded-lg sm:rounded-xl bg-[#11131a] p-0.5 sm:p-1 border border-white/5 mb-2.5 sm:mb-3.5 font-price text-[11px] sm:text-xs">
                  ...
                  <button
                    ...
                    className={`flex-1 py-1 sm:py-1.5 rounded-md sm:rounded-lg font-bold transition-all cursor-pointer flex items-center justify-center gap-1 ${
                      activeTab === 'telegram'
              ...
              {activeTab === 'signin' && (
                ...
                <div className="flex items-center gap-3 my-3">
                  ...
                  <span className="text-[10px] sm:text-xs text-[#8B90A0] font-price">
                    {lang === 'KM' ? 'ឬ' : 'OR'}
                  </span>
                  <div className="flex-1 h-px bg-white/10" />
                </div>
                ...
                <button
                  ...
                  disabled={isLoading}
                  className="w-full py-2.5 sm:py-3 bg-white hover:bg-[#f5f5f5] text-[#1f1f1f] rounded-lg sm:rounded-xl font-headline font-bold text-xs sm:text-sm shadow-md transition-all active:scale-98 cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2.5"
                >
                  ...
                  <span>{lang === 'KM' ? 'បន្តជាមួយ Google' : 'Continue with Google'}</span>
                </button>
                </>
              )}
          ...
          {activeTab === 'telegram' && (
            <div className="space-y-3">
              {telegramStep === 'enter-username' && (
                <>
                  <p className="text-xs text-[#8B90A0] leading-relaxed">
                    {lang === 'KM'
                      ? 'បញ្ចូល Telegram username របស់អ្នក ដើម្បីទទួលបានលេខកូដចូលប្រើលឿន។'
                      : "Enter your Telegram username to get a fast login code sent to you."}
                  </p>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8B90A0] text-sm">@</span>
                    <input
                      type="text"
                      value={telegramUsername}
                      onChange={(e) => setTelegramUsername(e.target.value)}
                      placeholder="username"
                      className="w-full bg-[#11131a] border border-white/10 focus:border-[#229ED9] rounded-lg sm:rounded-xl pl-7 pr-3.5 py-2.5 text-xs sm:text-sm text-white placeholder-[#505464] focus:outline-none transition-colors"
                    />
                  </div>
                  <button
                    type="button"
                    ...
                    disabled={isLoading}
                    className="w-full py-2.5 sm:py-3 bg-[#229ED9] hover:bg-[#1c8bc0] text-white rounded-lg sm:rounded-xl font-headline font-bold text-xs sm:text-sm shadow-md transition-all active:scale-98 cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    <Send className="w-4 h-4" />
                    <span>{isLoading ? (lang === 'KM' ? 'កំពុងផ្ញើ...' : 'Sending...') : lang === 'KM' ? 'ទទួលលេខកូដ' : 'Get Code'}</span>
                  </button>
                </>
              )}

              {telegramStep === 'needs-link' && (
                <>
                  <div className="p-3 rounded-xl bg-[#229ED9]/10 border border-[#229ED9]/30 text-xs text-[#9fe3fc] leading-relaxed">
                    {lang === 'KM'
                      ? `គណនី Telegram @${telegramUsername} មិនទាន់ភ្ជាប់ទេ។ សូមបើក Bot ខាងក្រោម ហើយចុច Start។`
                      : `Your Telegram account @${telegramUsername} isn't linked yet. Open the bot below and tap Start first.`}
                  </div>
                  <a
                    href={`https://t.me/${customerBotUsername}?start=login`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="w-full py-2.5 sm:py-3 bg-[#229ED9] hover:bg-[#1c8bc0] text-white rounded-lg sm:rounded-xl font-headline font-bold text-xs sm:text-sm shadow-md transition-all active:scale-98 cursor-pointer flex items-center justify-center gap-2"
                  >
                    <Send className="w-4 h-4" />
                    <span>{lang === 'KM' ? 'បើក Telegram Bot' : 'Open Telegram Bot'}</span>
                  </a>
                  <button
                    type="button"
                    onClick={() => setTelegramStep('enter-username')}
                    disabled={isLoading}
                    className="w-full py-2 text-[11px] sm:text-xs text-[#8B90A0] hover:text-white transition-colors cursor-pointer"
                  >
                    {lang === 'KM' ? '← ត្រឡប់ក្រោយ' : "I've linked it — try again"}
                  </button>
                </>
              )}

              {telegramStep === 'enter-code' && (
                <>
                  <p className="text-xs text-[#8B90A0] leading-relaxed">
                    {lang === 'KM'
                      ? `យើងបានផ្ញើលេខកូដ ៦ខ្ទង់ទៅ Telegram @${telegramUsername} របស់អ្នក។`
                      : `We sent a 6-digit code to your Telegram @${telegramUsername}.`}
                  </p>
                  <input
                    type="text"
                    inputMode="numeric"
                    maxLength={6}
                    value={telegramCode}
                    onChange={(e) => setTelegramCode(e.target.value.replace(/[^0-9]/g, ''))}
                    placeholder="123456"
                    className="w-full bg-[#11131a] border border-white/10 focus:border-[#229ED9] rounded-lg sm:rounded-xl px-3.5 py-2.5 text-center text-lg tracking-[0.5em] text-white placeholder-[#505464] focus:outline-none transition-colors font-mono"
                  />
                  <button
                    type="button"
                    onClick={handleTelegramVerifyCode}
                    disabled={isLoading || telegramCode.length !== 6}
                    className="w-full py-2.5 sm:py-3 bg-[#229ED9] hover:bg-[#1c8bc0] text-white rounded-lg sm:rounded-xl font-headline font-bold text-xs sm:text-sm shadow-md transition-all active:scale-98 cursor-pointer disabled:opacity-50"
                  >
                    {isLoading ? (lang === 'KM' ? 'កំពុងផ្ទៀងផ្ទាត់...' : 'Verifying...') : lang === 'KM' ? 'បញ្ជាក់ និងចូល' : 'Verify & Login'}
                  </button>
                  <button
                    type="button"
                    onClick={handleTelegramRequestCode}
                    disabled={isLoading || telegramCooldown > 0}
                    className="w-full py-2 text-[11px] sm:text-xs text-[#8B90A0] hover:text-white transition-colors cursor-pointer disabled:opacity-40"
                  >
                    {telegramCooldown > 0
                      ? (lang === 'KM' ? `ផ្ញើម្តងទៀតក្នុង ${telegramCooldown}s` : `Resend in ${telegramCooldown}s`)
